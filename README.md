...
- `security.trust_forwarded_ip_for_api_key_acl` enables legacy raw forwarded-header takeover (defaults to false); behind a reverse proxy, set it true explicitly if API key ACL needs XFF/client-IP headers, otherwise keep it false to enforce `server.trusted_proxies`, which should contain only the exact proxy CIDRs that connect directly to Sub2API
