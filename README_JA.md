...
- `security.trust_forwarded_ip_for_api_key_acl` - 従来の生転送ヘッダーによる上書きを制御（既定は無効）。リバースプロキシ配下で API Key ACL に XFF/クライアント IP ヘッダーが必要な場合は明示的に有効化。無効時は `server.trusted_proxies` を厳格に使用し、Sub2API に直接接続するプロキシの正確な CIDR のみを指定
