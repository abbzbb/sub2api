...
func RegisterGatewayRoutes(
	...
	// 之前，避免逐条路由手工维护链导致漏挂。self 的 postAuthSignal 挂在鉴权后。
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), postAuthSignal, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)
	}
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), postAuthSignal, groupModelAllowlist, compositeTarget, requireGroupAnthropic)
	{
