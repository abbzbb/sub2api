...
func TestAccountTestServiceGrokOAuthPaymentRequiredTemporarilyUnschedulesAccount(t *testing.T) {
	...
	require.Error(t, err)
	// Body is personal-team-blocked:spending-limit — rate-limit (not bare soft entitlement).
	require.Zero(t, repo.tempUnschedCalls)
	require.Equal(t, 1, repo.rateLimitedCalls)
...
func TestShouldFailoverGrokUpstreamErrorIncludesNotFound(t *testing.T) {
	...
	require.True(t, svc.shouldFailoverGrokUpstreamError(http.StatusNotFound, nil))
	require.False(t, svc.shouldFailoverUpstreamError(http.StatusNotFound))
	require.True(t, svc.shouldFailoverGrokUpstreamError(http.StatusPaymentRequired, nil))
