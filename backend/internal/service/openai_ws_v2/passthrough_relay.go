...
func Relay(
	...
	emitTurnComplete(options.OnTurnComplete, state, finalizePendingBareError(state, nowFn()), clientDisconnected)
...
func runUpstreamToClient(
	...
	for {
		...
		if err != nil {
			graceful := isDisconnectError(err)
			emitTurnComplete(onTurnComplete, state, finalizePendingBareError(state, nowFn()), graceful)
		...
		case coderws.MessageText:
			...
			if shouldFinalizePendingBareError(state, payload, eventType) {
				emitTurnComplete(onTurnComplete, state, finalizePendingBareError(state, nowFn()), false)
			}
		...
		if writeErr != nil {
			emitTurnComplete(onTurnComplete, state, observedEvent, isDisconnectError(writeErr))
			emitRelayTrace(onTrace, RelayTraceEvent{
