...
func (s *AccountTestService) grokTestAccessToken(ctx context.Context, account *Account) (string, error) {
	...
	case AccountTypeOAuth:
		...
		// accounts (including free-recovery pending) can still be probed by admins
		// (#4598), matching Codex/OpenAI test behavior.
...
func (s *AccountTestService) grokTestProxyURL(account *Account) (string, error) {
	return resolveAccountProxyURL(context.Background(), nil, account)
}

func (s *AccountTestService) requireGrokTestProxyURL(c *gin.Context, account *Account) (string, error) {
...
func (s *AccountTestService) reconcileGrokTestResponseState(
	...
	case http.StatusUnauthorized:
		if s.accountRepo != nil {
			...
			_ = s.accountRepo.SetTempUnschedulable(stateCtx, account.ID, until, "grok oauth token unauthorized")
		}
	default:
		if statusCode >= 500 && s.accountRepo != nil {
			...
			_ = s.accountRepo.SetTempUnschedulable(stateCtx, account.ID, until, "grok upstream temporary error")
		}
	}
}
...
func (s *AccountTestService) testGrokResponsesConnection(c *gin.Context, ctx context.Context, account *Account, authToken, testModelID string) error {
	...
	proxyURL, err := s.requireGrokTestProxyURL(c, account)
	if err != nil {
		return err
	}
...
func (s *AccountTestService) testGrokImageGeneration(c *gin.Context, ctx context.Context, account *Account, authToken, modelID, prompt, imageDataURL string) error {
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		proxyURL, proxyErr := s.requireGrokTestProxyURL(c, account)
		if proxyErr != nil {
			return proxyErr
		}
