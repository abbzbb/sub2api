...
var (
	...
	ErrChannelMonitorAccountNotSupportable = infraerrors.BadRequest(
		"CHANNEL_MONITOR_ACCOUNT_NOT_SUPPORTABLE", "linked account cannot serve as a quota data source (cn coding plan must be kimi/zhipu/minimax, cn payg must be kimi/deepseek, opencode_go must be a Go subscription account, openai requires an oauth account, anthropic requires oauth or setup-token)",
	)
