...
func (s *OpenAIGatewayService) proxyOpenAIWSHTTPBridgeTurn(
	...
	for {
		...
		if account.Platform == PlatformGrok {
			...
			canonicalModel := resolveGrokWSUpstreamModel(account, body, originalModel)
			s.handleGrokAccountUpstreamError(withGrokTeamRateLimitModel(ctx, canonicalModel), account, resp.StatusCode, resp.Header, respBody, canonicalModel)
	...
	for scanner.Scan() {
		...
		if eventType == "error" || eventType == "response.failed" {
			...
			// Grok response.failed must be written and the account reconciled before
			// returning an error. Turn-1 early failover applies only to error events
			// that have not been written yet. Upstream 429 may retry the same account.
