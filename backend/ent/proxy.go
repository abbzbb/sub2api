...
type Proxy struct {
	...
	// Optional proxy group this proxy belongs to (one group per proxy).
...
// was not loaded in eager-loading, or loaded but was not found.
func (e ProxyEdges) GroupOrErr() (*ProxyGroup, error) {
	if e.Group != nil {
		return e.Group, nil
...
func (*Proxy) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case proxy.FieldID, proxy.FieldPort, proxy.FieldBackupProxyID, proxy.FieldExpiryWarnDays, proxy.FieldGroupID:
...
// QueryGroup queries the "group" edge of the Proxy entity.
func (_m *Proxy) QueryGroup() *ProxyGroupQuery {
	return NewProxyClient(_m.config).QueryGroup(_m)
}
