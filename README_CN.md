...
- `security.trust_forwarded_ip_for_api_key_acl` 控制旧版原始转发头接管（默认关闭）；部署在反代后应保持关闭，并在 `server.trusted_proxies` 中仅填写直接连接 Sub2API 的精确代理 CIDR，由可信代理链解析 XFF。仅在确有兼容需要、源站无法被直接访问且代理会覆盖所有受信任客户端 IP 头时开启旧版接管
