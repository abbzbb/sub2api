...
- `security.trust_forwarded_ip_for_api_key_acl` 控制旧版原始转发头接管（默认关闭；仅在明确配置 `server.trusted_proxies` 后才应开启）。关闭后严格使用 `server.trusted_proxies`，其中只应填写直接连接 Sub2API 的精确代理 CIDR
