...
- `security.trust_forwarded_ip_for_api_key_acl` 控制旧版原始转发头接管（默认关闭）；若部署在反代后且 API Key ACL 需要识别 XFF/客户端 IP 头，请显式开启；关闭时严格使用 `server.trusted_proxies`，其中只应填写直接连接 Sub2API 的精确代理 CIDR
